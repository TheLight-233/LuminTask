...
public static class TaskTracker
    {
        ...
        static List<KeyValuePair<ILuminTaskSource, (string formattedType, int trackingId, DateTime addTime, string stackTrace)>> listPool = 
            new List<KeyValuePair<ILuminTaskSource, (string formattedType, int trackingId, DateTime addTime, string stackTrace)>>();

        static readonly WeakDictionary<ILuminTaskSource, (string formattedType, int trackingId, DateTime addTime, string stackTrace)> tracking = new 
            WeakDictionary<ILuminTaskSource, (string formattedType, int trackingId, DateTime addTime, string stackTrace)>();

        [Conditional("UNITY_EDITOR")]
        public static void TrackActiveTask(ILuminTaskSource task, int skipFrame)
        {
#if UNITY_EDITOR
            dirty = true;
        ...
        [Conditional("UNITY_EDITOR")]
        public static void RemoveTracking(ILuminTaskSource task)
        {
#if UNITY_EDITOR
            dirty = true;
            if (!EditorEnableState.EnableTracking) return;
        ...
        public static void ForEachActiveTask(Action<int, string, LuminTaskStatus, DateTime, string> action)
        {
            lock (listPool)
            {
